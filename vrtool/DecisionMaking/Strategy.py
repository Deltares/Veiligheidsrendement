...
class ParetoFrontier(Strategy):
    ...
    def evaluate(self, TrajectObject, SolutionsCollection, LCClist=False, PATH=False):
        ...
        for j in range(0, len(LCClist)):
            ...
            MIPModels.append(
                MIPObjects[-1].create_optimization_model(BudgetLimit=LCClist[j])
            )
            MIPModels[-1].solve()
            MIPResult = {}
            ...
            MIPObjects[-1].readResults(
                MIPResults[-1], MeasureTable=getMeasureTable(SolutionsCollection)
            )
            MIPObjects[-1].TakenMeasures.to_csv(
                PATH.joinpath("Pareto_LCC=" + str(np.int32(LCClist[j])) + ".csv")
            )
