...
class SectionDataValidator:
    def _valid_sensitivity(self, sensitivity: float) -> bool:
        _max_value = 1.0
        _lower_value = 0.0
        _within_lower_limit = sensitivity > _lower_value or isclose(
            sensitivity, 0.0, rel_tol=1e-9, abs_tol=1e-9
        )
        _within_upper_limit = sensitivity < _max_value or isclose(
            sensitivity, _max_value, rel_tol=1e-9, abs_tol=1e-9
        )
    ...
    def _validate_sensitivity(
        ...
        if not self._valid_sensitivity(section_data.sensitive_fraction_piping):
            yield add_sensitivy_error(
                "sensitive_fraction_piping", section_data.sensitive_fraction_piping
            )
        if not self._valid_sensitivity(section_data.sensitive_fraction_stability_inner):
            yield add_sensitivy_error(
                "sensitive_fraction_stability_inner",
                section_data.sensitive_fraction_stability_inner,
            )
    ...
    def validate(self, section_data: SectionData) -> ValidationReport:
        ...
        if not section_data:
            _validation_report.errors.append(
                ValidationError(
                    f"No valid value given for {SectionData.__name__}.", section_data
                )
            )
