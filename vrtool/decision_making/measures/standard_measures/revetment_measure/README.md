...
- `RevetmentMeasureSectionReliability`: Data object responsible to wrap the `SectionReliability` for a given `beta_target` and `transition_level` with its related `cost`. It also implements the `MeasureResultCollectionProtocol`.
- `RevetmentMeasureResultCollection`: A specialization of the `MeasureResultCollectionProtocol` to wrap all the `RevetmentMeasureSectionReliability`.
