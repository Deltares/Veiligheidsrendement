...
class StrategyBase:
    ...
    def set_investment_years(
        self,
        traject: DikeTraject,
        ids_to_import: list[tuple[int, int]],
        selected_measure_ids: dict[int, list[int]],
        ...
        for section in traject.sections:
            ...
            if section.name in solutions_dict:
                if section.with_measures:
                    for _ in range(len(solutions_dict[section.name].MeasureData)):
                        ...
                        self.indexCombined2single[section.name].append(
                            [_sel_measure_ids_copy[_run_id].pop(0)]
                        )
                        self.investment_years[section.name].append(
            ...
            beta_array_initial = {}
            new_section_reliability_df = pd.DataFrame(
                index=section.section_reliability.SectionReliability.index,
                columns=np.arange(0, np.max(self.T), 1),
            )
            for beta_type in section.section_reliability.SectionReliability.index:
                beta_array_initial[beta_type] = interp1d(
                    section.section_reliability.SectionReliability.columns.astype(int),
                    section.section_reliability.SectionReliability.loc[
                        beta_type
                    ].values,
                )(np.arange(0, np.max(self.T), 1))
                new_section_reliability_df.loc[beta_type, :] = beta_array_initial[
                    beta_type
                ]
            ...
            flattened_indexCombined2single = [
                item
                for sublist in self.indexCombined2single[section.name]
                for item in sublist
            ]
            ...
            if not any(flattened_indexCombined2single):
                ...
                for beta_type in section.section_reliability.SectionReliability.index:
                    beta_array_measures[beta_type] = np.empty(
                        (len(self.indexCombined2single[section.name]), np.max(self.T))
                    )
                    # interpolate the betas for each beta_type in solutions_dict[section.name].MeasureData
                    beta_array_measures[beta_type] = interp1d(
                        solutions_dict[section.name]
                        .MeasureData[beta_type]
                        .columns.astype(int),
                        solutions_dict[section.name].MeasureData[beta_type].values,
                    )(np.arange(0, np.max(self.T), 1))
                    ...
                    beta_array_investment_year[beta_type] = np.empty(
                        (len(self.indexCombined2single[section.name]), np.max(self.T))
                    )
                    for row in range(len(self.indexCombined2single[section.name])):
                        beta_array_investment_year[beta_type][row] = np.concatenate(
                            (
                                beta_array_initial[beta_type][
                                    : self.investment_years[section.name][row]
                                ],
                                beta_array_measures[beta_type][row][
                                    self.investment_years[section.name][row] :
                                ],
                            )
                        )
    ...
    def _step1combine(
        ...
        if "revetment" in measures_per_class:
            ...
            if any(combinedmeasures_indices):
                ...
                self.indexCombined2single[section.name] = (
                    self.indexCombined2single[section.name] + new_indices1
                )

        else:
            self.indexCombined2single[section.name] = (
                self.indexCombined2single[section.name] + combinedmeasures_indices
            )
    ...
    def make_optimization_input(self, traject: DikeTraject):
        ...
        for n in range(0, len(section_keys)):
            ...
            for sh_id in section_sh_ids:
                ...
                sh_years = (
                    self.options_height[section_keys[n]]
                    .loc[self.options_height[section_keys[n]].ID == sh_id]
                    .year
                )
                ...
                sg_years = (
                    self.options_geotechnical[section_keys[n]]
                    .loc[self.options_geotechnical[section_keys[n]].ID == sh_id]
                    .year
                )
                ...
                for i, sh_idx in enumerate(sh_indices):
                    for j, sg_idx in enumerate(sg_indices):
                        ...
                        if sh_years[sh_idx] == sg_years[sg_idx]:
                            self.LCCOption[n, sh_idx + 1, sg_idx + 1] = LCC_combined[
                                j, i
                            ]
