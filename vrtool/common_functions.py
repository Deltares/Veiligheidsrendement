...
def ld_write_object(file_path: Path, object_to_write):
    ...
    """
    f=open(file_path,'wb')
    newData = pickle.dumps(object_to_write, 1)
    f.write(newData)
    f.close()

def ld_read_object(pickle_file: Path):
    """
...
def draw_alpha_bar_plot(result_list: list, xlabels = None, Pvalues = None, method = 'MCS', suppress_ind = None, title = None):
    """
    ...
    if method == 'MCS':
        ...
        labels = result_list[idx].getImportanceFactors(ot.AnalyticalResult.CLASSICAL).getDescription()

    alphas = []
    for i in range(idx, len(result_list)):
        alpha = list(result_list[i].getImportanceFactors(ot.AnalyticalResult.CLASSICAL)) if method == 'FORM' else list(result_list[i].getImportanceFactors())
...
def calculate_r_exit(h_exit: float,k: float,d_cover,D,wl: float,Lbase,Lachter,Lfore = 0) -> float:
    """
    k = 0.0001736111
    h_exit = 2.5
    wl = 6.489
...
def adapt_input(grid_data: pd.DataFrame, monitored_sections: DikeSection, base_case: DikeTraject) -> list[DikeTraject]:
    _geo_risk_cases = []
        ...
        for j in _geo_risk_cases[-1].sections:
            if j.name in monitored_sections:
                for ij in list(j.Reliability.Mechanisms['Piping'].Reliability.keys()):
                    ...
                    wl = \
                    np.array(j.Reliability.Load.distribution.computeQuantile(1 - _geo_risk_cases[-1].GeneralInfo['Pmax']))[
                        0]
                    new_r_exit = calculate_r_exit(j.Reliability.Mechanisms['Piping'].Reliability[ij].Input.input['h_exit'],
            ...
            _geo_risk_cases[-1].GeneralInfo['P_scen'] = row['p']                # print(str(row['k']) + '   ' + str(new_r_exit))
    return _geo_risk_cases

def replace_names(strategy: StrategyBase, solutions_list: list[Solutions]) -> StrategyBase:
    ...
    for i in range(1, len(strategy.TakenMeasures)):
        ...
        name = solutions_list[section].measure_table.loc[solutions_list[section].measure_table['ID'] == id]['Name'].values
...
def read_batch_info(input_dir: Path) -> list[str]:
    ...
    _available_sections = [p for p in input_dir.joinpath('BaseData').iterdir() if p.is_file()]
    ...
    for row in _case_set.iterrows():
        if row[1]['CaseOn'] == 1:
            ...
            _case_measure_set = pd.DataFrame({'available': _measure_sets[row[1]['MeasureSet']].values})
            for run in range(1, row[1]['Runs']+1):
                _sections = np.random.randint(0, len(_available_sections), size=row[1]['Sections'])

                #make directory
                _case_path = input_dir.joinpath('{:02d}'.format(_case_no), '{:03d}'.format(run))
                ...
                for i in _sections:
                    copyfile(_available_sections[i],_case_path.joinpath(_available_sections[i].name))
                    book = load_workbook(_case_path.joinpath(_available_sections[i].name))
                    writer = pd.ExcelWriter(_case_path.joinpath(_available_sections[i].name),engine='openpyxl')
                    writer.book = book
                    writer.sheets = dict((ws.title,ws) for ws in book.worksheets)
                    _case_measure_set.to_excel(writer,'Measures',columns=['available'],index=False,startcol=3)
                    writer.save()
                ...
                for p in _case_path.iterdir():
                    if p.is_file():
                        ...
                        copyfile(_base_data_dir.joinpath(data.loc['Overflow']['Value']),_case_path.joinpath(data.loc['Overflow']['Value']))
                        copyfile(_base_data_dir.joinpath(data.loc['StabilityInner']['Value']),_case_path.joinpath(data.loc['StabilityInner']['Value']))
                        copyfile(_base_data_dir.joinpath(data.loc['Piping']['Value']),_case_path.joinpath(data.loc['Piping']['Value']))
                        copyfile(_base_data_dir.joinpath('Toetspeil',data.loc['LoadData']['Value']),_case_path.joinpath('Toetspeil',data.loc['LoadData']['Value']))
    return _case_list
