...
class Overflow:
    ...
    def calculate_simple(
        ...
        if mechanism_input.q_c[0] != mechanism_input.q_c[-1:]:
            beta_hc = interpolate.interp2d(
                mechanism_input.h_c,
                mechanism_input.q_c,
            ...
            beta = np.min(
                [
                    beta_hc(
                        mechanism_input.corrected_crest_height, mechanism_input.q_crest
                    ),
                    8.0,
                ]
            )
            ...
            beta = np.min([beta_hc(mechanism_input.corrected_crest_height), [8.0]])
        Pf = beta_to_pf(beta)
