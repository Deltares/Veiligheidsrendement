...
class RunStepOptimizationValidator(RunStepValidator):
    ...
    def get_test_measure_results(
        self, valid_vrtool_config: VrtoolConfig
    ) -> list[orm_models.MeasureResult]:
        _connected_db = open_database(valid_vrtool_config.input_database_path)
        _mr_list = list(orm_models.MeasureResult.select())
        _connected_db.close()
        return _mr_list
    ...
    def get_test_measure_result_with_investment_year(
        ...
        # Example of how to concatenate queries.
        # _measure_results_with_soil_query = (orm_models.MeasureResult.select()
        #     .join(orm_models.MeasurePerSection)
        #     .join(orm_models.Measure)
        #     .join(orm_models.MeasureType)
        #     .where(
        #         (orm_models.MeasureType.name.contains("Soil reinforcement")) & (orm_models.MeasureResult.in_(measures_results)))
        # )
        ...
        for _measure_result in measures_results:
            if _measure_result.measure_per_section.measure.year == 20:
                # NOTE: Remove this condition once databases are up to date.
                continue
