...
class TestSetInvestmentYear:
    def _get_measure(self, year: int, revetment_params: list[float], probabilities: list[float]) -> ShMeasure:
        """
        ...
        Args:
            ...
            probabilities (list[float]): list with probabilities for years 0 and 50

        Returns:
            ShMeasure: a revetment measure based on the given arguments
        """
        _mech_per_year1 = MechanismPerYear(MeasureTypeEnum.REVETMENT, 0, probabilities[0])
        _mech_per_year2 = MechanismPerYear(MeasureTypeEnum.REVETMENT, 50, probabilities[1])
    ...
    def test_two_measures(self):
        # setup
        _measure_list = [self._get_measure( 0, [0.0, 0.0, 0.0], [0.8, 0.7]),
                         self._get_measure(20, [4.0, 2.0, 0.0], [0.6, 0.5])]
    ...
    def test_three_measures(self):
        ...
        _measure_list = [self._get_measure( 0, [0.0, 0.0, 0.0], [0.8, 0.7]),
                         self._get_measure(20, [4.0, 2.0, 0.0], [0.6, 0.5]),
                         self._get_measure(30, [4.0, 2.0, 0.0], [0.6, 0.5])]
        ...
        # year 30 for _measure3 is an interpolated value copied from _measure1:
        assert _measure_list[2].mechanism_year_collection.get_probability(
            MeasureTypeEnum.REVETMENT, 30
        ) == py.approx(0.74257, abs=1e-5)

        # _measure1 is extended with two years (20 and 30):
        assert len(_measure_list[0].mechanism_year_collection.probabilities) == 4

        # _measure2 is extended with two years (20 and 21):
        assert len(_measure_list[1].mechanism_year_collection.probabilities) == 4

        # _measure3 is extended with two years (30 and 31):
        assert len(_measure_list[2].mechanism_year_collection.probabilities) == 4

    def test_measures_without_year_zero(self):
        # setup
        _measure_list = [self._get_measure(20, [4.0, 2.0, 0.0], [0.6, 0.5])]
        ...
        # check result
        assert "zero measure not found for this type: REVETMENT" == str(exceptionInfo.value)
