...
class TestMigrateDatabaseController:
    def test_initialize(self, valid_conversion_scripts: Path):
        # 1. Run test
        _migrate_db = MigrateDatabaseController(valid_conversion_scripts)
    ...
    def test_is_database_compatible(
        ...
        # 2. Run test
        _result = MigrateDatabaseController.is_database_compatible(valid_conversion_db)

        # 3. Verify expectations
        assert _result == expected

    def test__apply_migration_script(
        self, valid_conversion_scripts: Path, valid_conversion_db: Path
    ):
        # 1. Define test data
    ...
    def test__apply_erroneous_script_raises(
        self, valid_conversion_scripts: Path, valid_conversion_db: Path
    ):
        # 1. Define test data
        _script = valid_conversion_scripts.joinpath("wrong_script.txt")
    ...
    def test_migrate_single_db(
        ...
        _migrate_db.migrate_single_db(valid_conversion_db)
