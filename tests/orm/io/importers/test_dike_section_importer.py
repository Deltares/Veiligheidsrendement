...
class TestDikeSectionImporter:
    ...
    @pytest.mark.usefixtures("db_fixture")
    def test_given_sensitive_fraction_piping_greater_than_1_when_import_orm_then_raises(
        self, valid_config: VrtoolConfig
    ):
        # 1. Define test data.
        _importer = DikeSectionImporter(valid_config)
        _section_data: SectionData = SectionData.get_by_id(1)
        _section_data.sensitive_fraction_piping = 1.000000001
        _section_data.sensitive_fraction_stability_inner = 0

        # 2. Run test
        with pytest.raises(ValueError) as exc_err:
            _importer.import_orm(_section_data)

        # 3. Verify expectations.
        assert (
            str(exc_err.value)
            == "'sensitive_fraction_piping' should be a real value in the [0.0, 1.0] limit, but got '1.000000001'."
        )

    @pytest.mark.usefixtures("db_fixture")
    def test_given_no_section_data_when_import_orm_then_raises(
        self, valid_config: VrtoolConfig
    ):
        # 1. Define test data.
        _importer = DikeSectionImporter(valid_config)
    ...
    @pytest.mark.usefixtures("db_fixture")
    def test_given_sensitive_fraction_stability_inner_greater_than_1_when_import_orm_then_raises(
        self, valid_config: VrtoolConfig
    ):
        # 1. Define test data.
        _importer = DikeSectionImporter(valid_config)
        _section_data: SectionData = SectionData.get_by_id(1)
        _section_data.sensitive_fraction_piping = 0
        _section_data.sensitive_fraction_stability_inner = 1.000000001
        ...
        assert (
            str(exc_err.value)
            == "'sensitive_fraction_stability_inner' should be a real value in the [0.0, 1.0] limit, but got '1.000000001'."
        )
