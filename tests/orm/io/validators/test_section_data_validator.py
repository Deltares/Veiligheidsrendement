...
class TestSectionDataValidator:
    ...
    def test_given_no_section_data_when_validate_then_generates_one_error(self):
        ...
        assert _report.errors[0].error_message == "No valid value given for SectionData."


    @pytest.mark.parametrize("invalid_value", [
        pytest.param(-0.00001, id="Under lower limit"), 
        pytest.param(1.0001, id="Over upper limit")])
    def test_given_invalid_sensitivities_when_validate_then_generates_validation_errors(self, invalid_value: float):
        # 1. Given.
        _validator = SectionDataValidator()
        ...
        def assert_error(report_error: ValidationError, context_object: str):
            ...
            assert report_error.error_message == f"'{context_object}' should be a real value in the [0.0, 1.0] limit, but got '{invalid_value}'."
