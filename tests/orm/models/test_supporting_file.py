...
class TestSupportingFile:
    ...
    def test_initialize_with_database_fixture(self, empty_db_fixture):
        ...
        _mech_per_section = MechanismPerSection.create(section=_test_section, mechanism=_test_mechanism)
        _scenario = ComputationScenario.create(mechanism_per_section=_mech_per_section, computation_type = _computation_type, computation_name="Test Computation", scenario_name="test_name", scenario_probability=0.42, probability_of_failure=0.24)
        _computation_type = ComputationType.create(name="TestComputation")

        # 2. Run test.
        _support_file = SupportingFile.create(computation_scenario=_scenario, filename="JustAFile")
