...
def main():
    ...
    PATH = Path(r'd:\wouterjanklerk\My Documents\00_PhDgeneral\03_Cases\01_Rivierenland '
                r'SAFE\WJKlerk\SAFE\data\SAFE_16-4_CACAIE')
    case = 'Case_test'
    ...
    #                         years = [25], labels_limited=True, system_rel=True,
    #                         custom_name='Assessment_2050.png', show_xticks=True,
    #                         title_in='(a) \n' + r'$\bf{Initial~reliability~in~2050}$')
    #
    # #pane 2: reliability in 2050, with Greedy optimization
    # TestCase.plotAssessment(PATH=directory, fig_size=figsize, flip='on',
    #                         years = [25], labels_limited=True,system_rel=True,
    #                         custom_name='GreedyStrategy_2050.png', reinforcement_strategy=AllStrategies[0], greedymode = 'SafetyStandard',
    #                         show_xticks=False, title_in='(c)\n' + r'$\bf{Optimized~investment}$ - Reliability in 2050',colors=optimized_colors)
    #
    # #pane 3: reliability in 2050, with Target Reliability Approach
    # TestCase.plotAssessment(PATH=directory, fig_size=figsize, flip='on',
    #                         years = [25], labels_limited=True,system_rel=True,
    #                         custom_name='TargetReliability_2050.png', reinforcement_strategy=AllStrategies[1],
    #                         show_xticks=False, title_in='(e) \n' + r'$\bf{Target~reliability~based~investment}$ -  Reliability in 2050',colors=targetrel_colors)
    #
    ...
    #                               title_in='(b) \n' + r'$\bf{Optimized~investment}$ - Measures',colors=optimized_colors)
    ...
    AllStrategies[0].plotBetaCosts(TestCase, t=50,
                                   fig_id=102, symbolmode=True, markersize = 9, final_step = AllStrategies[0].OptimalStep,color=twoColors[0], series_name='Optimized ' \
                                                                                                                                                    'investment',
                                   MeasureTable=MeasureTable, beta_or_prob='beta', outputcsv=PATH)
    AllStrategies[1].plotBetaCosts(TestCase, t=50,
                                   fig_id=102, symbolmode=True, markersize = 9, color=twoColors[1], series_name='Target reliability based investment',
                                   MeasureTable=MeasureTable,last=True, beta_or_prob='beta', outputcsv=PATH)
    plt.savefig(directory.joinpath('Priority order Beta vs LCC_2075.png'),dpi=300,bbox_inches='tight',format='png')
