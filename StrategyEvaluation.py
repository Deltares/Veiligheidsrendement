...
def split_options(options):
    ...
    for i in options:
        ...
        options_height[i] = options_height[i].loc[(options[i]['type'] == 'Diaphragm Wall')| (options[i]['type'] == 'Custom') | (options[i]['dberm'] == 0)]
...
def OverflowBundling(Strategy, init_overflow_risk,existing_investment,
    ...
    sorted_sh = np.empty((np.size(LifeCycleCost, axis=0), np.size(LifeCycleCost, axis=1)),dtype=np.int32)
    sorted_sh.fill(999)
    ...
    for i in range(0, len(traject.Sections)):
        ...
        if any(existing_investments[i,:]>0):
            ...
            if isinstance(GeotechnicalOptions.iloc[investment_id]['year'].values[0],list):
                #take last: this should be the soil reinforcement.
                if GeotechnicalOptions.iloc[investment_id]['type'].values[0][0] == 'Soil reinforcement':
                    print('Warning, first combined measure is a soil reinforcement. This might not result in the intended behaviour')
                current_type = GeotechnicalOptions.iloc[investment_id]['type'].values[0][1]
                extra_type = GeotechnicalOptions.iloc[investment_id]['type'].values[0][0]
                current_class = GeotechnicalOptions.iloc[investment_id]['class'].values[0]
                year_of_investment = GeotechnicalOptions.iloc[investment_id]['year'].values[0][-1]
            else:
                year_of_investment = GeotechnicalOptions.iloc[investment_id]['year'].values[0]
        ...
        if current_type in ['Diaphragm Wall', 'Custom']:
            ...
            LCC = LifeCycleCost[i, existing_investments[i,0], existing_investments[i,1]]
        ...
        #Soil reinforcement can only remain of same class. For t=20 it can be moved forward in time:
        elif (current_type == 'Soil reinforcement') and (extra_type == None): #soil reinforcement with stability screen
            ...
            if year_of_investment == 0:
                ...
                subset = HeightOptions.loc[(HeightOptions['type'] == 'Soil reinforcement') & (HeightOptions['class'] == current_class)]
                sh_opts = subset.loc[subset['year'] == year_of_investment].index.values + 1
                LCCs = LifeCycleCost[i, sh_opts,existing_investments[i,1]]
                sorted_sh[i, 0:len(LCCs)] = sh_opts[np.argsort(LCCs)]
                sorted_sh[i, 0:len(LCCs)] = np.where(np.sort(LCCs) > 1e60, 999, sorted_sh[i, 0:len(LCCs)])
                ...
                sorted_sh[i,0:len(orderedLCCs)] = np.where(orderedLCCs > 1e60, 999, sh_opts[order[0]])
        ...
        #For a stability screen, we should check if it can be extended with a berm or crest. Note that not allowing this might result in a local optimum.
            ...
            beta_investment_year = GeotechnicalOptions['StabilityInner',year_of_investment].loc[GeotechnicalOptions['ID'] == GeotechnicalOptions['ID'][investment_id]].values[0]
            ID_allowed = GeotechnicalOptions.loc[GeotechnicalOptions['StabilityInner',year_of_investment]==beta_investment_year].loc[GeotechnicalOptions['class']=='full']['ID'].values
            if len(ID_allowed) >0:
                ID_allowed = np.unique(ID_allowed)
                if len(ID_allowed) > 1:
                    ids = []
                    for ID_all in ID_allowed:
                        ids.append(np.argwhere(GeotechnicalOptions['ID'].values == ID_all))
                ...
                sorted_sh[i, :] = np.where(np.sort(LCCs) > 1e60, 999, sorted_sh[i, 0:len(LCCs)])
        elif (current_type == 'Vertical Geotextile') or (extra_type == 'Vertical Geotextile'):
            if extra_type == 'Vertical Geotextile':
            #There is already a soil reinforcement, we should keep the berm
                current_berm = GeotechnicalOptions.iloc[existing_investments[i][1]-1].dberm.values[0]
                subset = GeotechnicalOptions.loc[(GeotechnicalOptions['dberm']==current_berm) & (GeotechnicalOptions['class']=='combined')]
                if GeotechnicalOptions.iloc[existing_investments[i][1]-1].year.values[0][1]==0:
                ...
                subset = GeotechnicalOptions.loc[(GeotechnicalOptions['dberm']==0.0) & (GeotechnicalOptions['class']=='combined')]
            ...
            sorted_sh[i,:] = np.where(np.sort(np.min(LCC_1, axis=1)) > 1e60, 999,sorted_sh[i, :])
    ...
    while run_number < 100:
        ...
        LCC_values[ind_weakest] = np.min(LifeCycleCost[ind_weakest,sorted_sh[ind_weakest,index_counter[ind_weakest]],
