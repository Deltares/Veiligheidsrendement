...
def compare_investment_strategy(wb_TC, wb_OI, investment_limit, number_of_sections, output_file_path, measure_file_path):
    investment_TC = 0
    wb_TC['params'] = np.where(wb_TC['params'] == 'yes', '-', wb_TC['params'])
    wb_OI['params'] = np.where(wb_OI['params'] == 'yes', '-', wb_OI['params'])
    ...
    with pd.ExcelWriter(output_file_path) as writer:
        ...
        df_TC_below_limit = pd.DataFrame(columns=['Dijkvak', 'Prioritering', 'TC Maatregel Tot Limiet', 'Kosten', 'Dimensies'])
        df_TC_above_limit = pd.DataFrame(columns=['Dijkvak', 'TC Maatregel Na Limiet', 'Kosten', 'Dimensies'])
        ...
        for i, j in enumerate(range(len(wb_TC['Section']))):
            ...
            if len(measure_id) == 1:
                ...
                dimensions = wb_TC['params'][j+1][1:-1].split(', ')
            ...
            if investment_TC <= investment_limit:
                ...
                df_TC_below_limit.iloc[index, [2, 4]] = [measure_description, dimensions]
                ...
                df_TC_above_limit.iloc[index, [1, 3]] = [measure_description, dimensions]
        ...
        df_OI = pd.DataFrame({'Dijkvak': wb_OI['Section'].sort_values().tolist(), 'OI Maatregel': '', 'Kosten': '', 'Dimensies': ''})
        ...
        for i in range(len(wb_OI['Section'])):
            ...
            if len(measure_id) == 1:
                measure_description = measures[measures['ID'] == int(measure_id[0])]['Name'].values[0]
                dimensions = wb_OI['params'][i+1]
            else:
                measure_description = measures[measures['ID'] == int(measure_id[0])]['Name'].values[0] + ' + ' + measures[measures['ID'] == int(measure_id[1])]['Name'].values[0]
                dimensions = wb_OI['params'][i+1][1:-1].split(', ')
                dimensions = dimensions[1] + ', ' + dimensions[2]

            index = df_OI[df_OI['Dijkvak'] == wb_OI['Section'][i+1]].index
            df_OI.iloc[index, 1:] = [measure_description, wb_OI['LCC'][i+1], dimensions]
        ...
        df_OI.to_excel(writer, sheet_name='TC and OI Comparison', header=False, index=False, startrow=1, startcol=9, engine="openpyxl")

        # Add Header Format
        wb = writer.book
        column_header = ['Dijkvak', 'Prioritering', 'TC Maatregel Tot Limiet', 'Kosten', 'Dimensies', 'TC Maatregel Na Limiet', 'Kosten', 'Dimensies', '', 'Dijkvak', 'OI Maatregel', 'Kosten', 'Dimensies']
...
def main():
    ...
    traject = '16-3'
    casename = 'SAFE_v3'
    path = Path(r'd:\wouterjanklerk\My Documents\00_PhDgeneral\03_Cases\01_Rivierenland SAFE\WJKlerk\SAFE\data\SAFE_' + traject + '_augustus')
    directory = path.joinpath('Case_' + casename, 'results')
    measure_path = Path(r'd:\wouterjanklerk\My Documents\00_PhDgeneral\03_Cases\01_Rivierenland SAFE\WJKlerk\SAFE\data\InputFiles\SAFEInput').joinpath(traject, 'Input')
    ...
    number_of_sections = len([i for i in path.glob('*.xlsx')])
    ...
    compare_investment_strategy(wb_TC, wb_OI, investment_limit, number_of_sections, directory.joinpath(output_file), measure_path.joinpath(measure_file))
