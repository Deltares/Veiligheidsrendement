...
def measure_per_section_to_df(measures_per_section, section_parameters):
    ...
    for section in section_parameters.keys():
        ...
        df = pd.concat([df, pd.DataFrame({'section_id':section,'name': _name, 'LCC': _LCC, 'investment_years': _investment_years, **_parameters}, index = [0])], ignore_index = True)
...
def plot_difference_in_betas(betas_per_section_and_mechanism, has_revetment, run_1 = 'reference', run_2 = 'result'):
    ...
    for mechanism in [MechanismEnum.OVERFLOW, MechanismEnum.PIPING, MechanismEnum.STABILITY_INNER, MechanismEnum.REVETMENT]:
        if has_revetment or mechanism != MechanismEnum.REVETMENT:
            ax.plot(betas_per_section_and_mechanism.loc[(betas_per_section_and_mechanism['mechanism'] == mechanism) & (betas_per_section_and_mechanism['run']==run_1)].beta,
                    betas_per_section_and_mechanism.loc[(betas_per_section_and_mechanism['mechanism'] == mechanism) & (betas_per_section_and_mechanism['run']==run_2)].beta, 
                    'o', label = mechanism)
    ax.legend()
    ax.set_xlabel(f'Beta {run_1}')
    ax.set_ylabel(f'Beta {run_2}')

def plot_difference_in_betas_per_section(betas_per_section_and_mechanism, has_revetment, run_1 = 'reference', run_2 = 'result'):
    ...
    #markers
    markers = ['d', 'o', 's', 'v']
    for count, mechanism in enumerate([MechanismEnum.OVERFLOW, MechanismEnum.PIPING, MechanismEnum.STABILITY_INNER, MechanismEnum.REVETMENT]):
        if has_revetment or mechanism != MechanismEnum.REVETMENT:
            ax.plot(betas_per_section_and_mechanism.loc[(betas_per_section_and_mechanism['mechanism'] == mechanism) & (betas_per_section_and_mechanism['run']==run_1)].section_id,
                    betas_per_section_and_mechanism.loc[(betas_per_section_and_mechanism['mechanism'] == mechanism) & (betas_per_section_and_mechanism['run']==run_2)].beta.values - 
                    betas_per_section_and_mechanism.loc[(betas_per_section_and_mechanism['mechanism'] == mechanism) & (betas_per_section_and_mechanism['run']==run_1)].beta.values, 
