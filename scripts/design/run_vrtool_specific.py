...
def rerun_database(_vr_config: VrtoolConfig, rerun_all: bool = False, run_name: str = None):
    if rerun_all:
    ...
    if run_name is None:
        raise ValueError("Please provide a unique run name")
    api.run_optimization(optimization_name=run_name,
...
def run_dsn_lenient_and_stringent(_vr_config: VrtoolConfig, run_strict: bool = False):
    ...
    if not run_strict:
        ...
        _vr_config.requirements_file = _vr_config.input_directory.joinpath("requirements_strict.csv")
