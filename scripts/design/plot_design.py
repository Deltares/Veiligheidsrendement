...
def plot_1():
    ...
    optimization_steps_vakspecifiek = {case: get_optimization_steps_for_run_id(results_dir.joinpath(database), run_id) for case,(database, run_id) in databases.items()}
    considered_tc_step_vakspecifiek = {case: get_minimal_tc_step(optimization_steps_vakspecifiek)-1 for case, optimization_steps_vakspecifiek in optimization_steps_vakspecifiek.items()}
    ...
    for count, run in enumerate(optimization_steps_vakspecifiek.keys()):
        ...
        ax.plot(optimization_steps_vakspecifiek[run][considered_tc_step_vakspecifiek[run]]['total_lcc'], optimization_steps_vakspecifiek[run][considered_tc_step_vakspecifiek[run]]['total_risk'], markers[count], color = colors[count])

        #add the point for 1/1000 * .52 if veilgiheidsrendement
        if run == 'Veiligheidsrendement':
            ax.plot(optimization_steps_vakspecifiek[run][step_idx_pf_2075]['total_lcc'], optimization_steps_vakspecifiek[run][step_idx_pf_2075]['total_risk'], markers[count], color = colors[count])
...
def plot_combined_measure_figure(cost: list,
                                 pf_traject:list,
                                 cost_vrm: list,
                                 pf_2075_vrm: list,
    ...
    ax.plot(vrm_optimum_point[0], vrm_optimum_point[1],
            marker='o', color=colors[0], label='VRM optimum')
