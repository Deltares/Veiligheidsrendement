...
def main():
    ...
    AllStrategies[0].plotBetaCosts(TrajectObject, save_dir=case_config.directory, fig_id=101,
                                   series_name=case_config.design_methods[0], MeasureTable=MeasureTable, color='b')
    AllStrategies[1].plotBetaCosts(TrajectObject, save_dir=case_config.directory, fig_id=101,
                                   series_name=case_config.design_methods[1], MeasureTable=MeasureTable, last='yes')
    plt.savefig(case_config.directory.joinpath('Priority order Beta vs LCC_' + str(config.t_0) + '.png'), dpi=300,
                bbox_inches='tight', format='png')

    # LCC-beta for t=50
    plt.figure(102, figsize=figure_size)
    AllStrategies[0].plotBetaCosts(TrajectObject, save_dir=case_config.directory, t=50, fig_id=102,
                                   series_name=case_config.design_methods[0], MeasureTable=MeasureTable, color='b')
    AllStrategies[1].plotBetaCosts(TrajectObject, save_dir=case_config.directory, t=50, fig_id=102,
                                   series_name=case_config.design_methods[1], MeasureTable=MeasureTable, last='yes')
    plt.savefig(case_config.directory.joinpath('Priority order Beta vs LCC_' + str(config.t_0 + 50) + '.png'), dpi=300,
                bbox_inches='tight', format='png')

    # Costs2025-beta
    plt.figure(103, figsize=figure_size)
    AllStrategies[0].plotBetaCosts(TrajectObject, save_dir=case_config.directory, cost_type='Initial', fig_id=103,
                                   series_name=case_config.design_methods[0], MeasureTable=MeasureTable, color='b')
    AllStrategies[1].plotBetaCosts(TrajectObject, save_dir=case_config.directory, cost_type='Initial', fig_id=103,
                                   series_name=case_config.design_methods[1], MeasureTable=MeasureTable, last='yes')
    plt.savefig(case_config.directory.joinpath('Priority order Beta vs Costs_' + str(config.t_0 + 50) + '.png'),
                dpi=300, bbox_inches='tight', format='png')

    AllStrategies[0].plotInvestmentLimit(TrajectObject, investmentlimit=20e6,
                                         path=case_config.directory.joinpath('figures'), figure_size=(12, 6), years=[0],
                                         flip=True)

    ## write a LOG of all probabilities for all steps:
    AllStrategies[0].writeReliabilityToCSV(path=case_config.directory.joinpath('results', 'investment_steps'),
                                           type=case_config.design_methods[0])
    AllStrategies[1].writeReliabilityToCSV(path=case_config.directory.joinpath('results', 'investment_steps'),
                                           type=case_config.design_methods[1])
    ...
    for count, Strategy in enumerate(AllStrategies):
        ...
        pd.DataFrame(ps, columns=range(100)).to_csv(
            path_or_buf=case_config.directory.joinpath('results', 'investment_steps',
                                                       'PfT_' + case_config.design_methods[count] + '.csv'))
